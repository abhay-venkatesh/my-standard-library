...
--- AllOne Data Structure Design ---

We place keys in buckets. The Buckets each refer to a value,
and the buckets are sorted by their value.

The keys in the buckets are placed in hash sets.
We map each key to a bucket in an unordered_map so we can
get O(1) access to keys.
